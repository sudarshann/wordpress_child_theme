...
class SB {
    ...
    public static function load_localize_script($script_name, $dependancies = array(), $object_name = '', $data = '') {
        ...
        if (!file_exists(SB_ASSETS_SCRIPT_PATH . $script_name . '.js')) {
            return false;
        }

        wp_enqueue_script(
                SB_CLASSES_NAMESPACE . '-' . $script_name,
                SB_ASSETS_SCRIPT_URI . $script_name . '.js',
                $dependancies,
                filemtime(SB_ASSETS_SCRIPT_PATH . $script_name . '.js')
        );
    ...
    public static function load_style($style_name, $dependancies = array()) {
        ...
        if (!file_exists(SB_ASSETS_STYLE_PATH . $style_name . '.css')) {
            return false;
        }

        wp_enqueue_style(
                SB_CLASSES_NAMESPACE . '-' . $style_name,
                SB_ASSETS_STYLE_URI . $style_name . '.css',
                $dependancies,
                filemtime(SB_ASSETS_STYLE_PATH . $style_name . '.css')
        );
    }
    ...
    public static function load_classes($class_list = []) {
        ...
        foreach ($class_list as $class_name) {
            $file = SB_CLASSES_PATH . SB_CLASSES_NAMESPACE . '/' . $class_name;
            if (!file_exists($file)) {
                return;
            }
    ...
    public static function load_template($plugin_template_path, $data = array()) {
        if (file_exists(SB_TEMPLATES_PATH . $plugin_template_path)) {
            require SB_TEMPLATES_PATH . $plugin_template_path;
        }
    }

    public static function get_image_url($image_path) {
        return SB_ASSETS_IMAGE_URI . $image_path;
    }
    ...
    public static function get_api_namespace() {
        return sanitize_title(SB_CLASSES_NAMESPACE) . '/' . SB_API_VERSION;
    }
